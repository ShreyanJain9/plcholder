...
#[rustler::nif]
fn decompress_public_key<'a>(env: Env<'a>, public_key: Binary<'a>) -> Term<'a> {
    let public_key = match hex::decode(public_key.as_slice()) {
        Ok(key) => key,
        Err(e) => {
            return (
    ...
    let decompressed_key = hex::encode(public_key.to_encoded_point(false).as_bytes());
    (atoms::ok(), decompressed_key).encode(env)
}
...
rustler::init!(
    "Elixir.Plcholder.K256.Internal",
    [create_public_key, compress_public_key, sign_message, verify_signature, decompress_public_key]
);
